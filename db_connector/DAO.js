...
const { selectDatabase } = require('./DB_functions');

module.exports = function (connectionData, schema) {
	...
	this.createConnection = (connObject) => {
		this.pool = mysql.createPool(connObject);
	};
			...
			this.pool.getConnection(async (connErr, conn) => {
				if (connErr) {
					...
					if (this.currentDatabase !== null) {
						await selectDatabase(conn, this.currentDatabase);
					}
