...
module.exports = function (connectionData) {
	...
	this.createBackup = async (database, filePath, opts = {}) => {
		await mysqldump({
			...
			dump: {
				...
				data: {
					format: false,
					maxRowsPerInsertStatement:
						opts.maxRowsPerInsertStatement || 10,
				},
				schema: {
					format: false,
				},
	...
	this.runQueryTransaction = async (sql, database, opts = {}) => {
		const backupPath =
			opts.backupPath !== undefined && typeof opts.backupPath === 'string'
				? opts.backupPath
				: path.join(__dirname, '..', `backup-${uniqid()}.sql`);
		await this.createBackup(database, backupPath);
		try {
			await this.runMultipleStatements(sql, database);
			if (
				!(
					opts.backupPath !== undefined &&
					typeof opts.backupPath === 'string'
				)
			) {
	 ...
	 * @returns {boolean||Error} true if success, object error otherwise
	 */
	this.runQueryTransactionFromFile = async (
		filePath,
		database,
		opts = {}
	) => {
