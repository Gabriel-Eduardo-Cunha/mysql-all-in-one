...
export class DataAccessObject {
   ...
   */
  public async dumpDatabase(database: string, filePath: string): Promise<void> {
      ...
      switch (returnMode) {
        ...
        case "firstColumn":
          const columnValues = resultSet.map((row) =>
            typeof row === "object" ? Object.values(row)?.[0] : undefined
          );
        ...
        case "firstValue":
          ...
          return firstRowValues.length !== 0 && isSqlValues(firstRowValues[0])
            ? firstRowValues[0]
            : null;
        case "specific":
      ...
      if (isInsertRows(rows)) {
        ...
        if (
          ...
          for (const rowsGroup of rowsGroups) {
            ...
            await this.executionMethod(preparedStatement, database, conn);
          }
          return null;
        }
        const insertedIds = [];
  ...
  public async upsert(
    ...
    try {
      ...
      for (const row of rows) {
        ...
        const insertedId = await this.insert(table, row, { database }, conn);
                ...
                conn.beginTransaction((err) => {
                  ...
                  return res({
                    select: (selectOpts, opts) =>
                      this.select(selectOpts, opts, conn),
                    delete: (
                      table: string,
                      whereOpts?: ConditionOptions,
                      opts?: DeleteOptions
                    ) => this.delete(table, whereOpts, opts, conn),
                    insert: (
                      table: string,
                      rows: InsertRows,
                      opts?: InsertOptionsDAO & InsertOptions
                    ) => this.insert(table, rows, opts, conn),
                    update: (
                      table: string,
                      values: UpdateValues,
                      whereOpts?: ConditionOptions,
                      opts?: UpdateOptions
                    ) => this.update(table, values, whereOpts, opts, conn),
                    upsert: (
                      table: string,
                      rows: UpsertRow,
                      opts?: UpsertOptions
                    ) => this.upsert(table, rows, opts, conn),
  ...
  private async connUseDatabase(conn: PoolConnection, database?: string) {
    const shouldChangeDatabase =
      isNotEmptyString(database) && database !== this.connectionData.database;
  ...
  private connExecute(
    ...
    return new Promise((resolve, reject) => {
      conn.execute(
        ...
        preparedStatement.values?.map((e) => (e === undefined ? null : e)),
        (err, result) => {
          if (err) return reject(err);
          resolve(result);
        }
